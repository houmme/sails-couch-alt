...
module.exports = (function() {
  ...
  var adapter = {
    ...
    //
    syncable: false,
    reservedAttributes: ['id', 'rev', 'doc_type', 'doc_hash', 'attachments'],
   ...
   */
    ...
    getAttachment: function(connectionName, collectionName, values, cb){
      logger.debug("Get Attachment ", connectionName, collectionName, values);
      var db = connections[connectionName];
      var obj = helper.prepare(values, collectionName, definitions[collectionName]);
      return helper.getAttachmentData(db, obj, collectionName + '/' + obj._id, cb);
    },

    ensureView: function(connectionName, collectionName, values, cb) {
      if (!(values && values.view_name && values.view_func))
        return cb(new Error('No view name or view function defined'));
      var db = connections[connectionName];
      var viewObj = helper.buildView(values.view_name, values.view_func);
      helper.createView(db, viewObj, collectionName, cb, true);
    },

    createOrUpdate: function(connectionName, collectionName, values, cb, check_hash = false) {
      logger.debug("Create or Update ", connectionName, collectionName, values);
      var db = connections[connectionName];
      var obj = helper.prepare(values, collectionName, definitions[collectionName]);
      return helper.upsert(db, obj, collectionName + '/' + obj._id, cb, check_hash);
    },
