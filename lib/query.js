...
module.exports = {
    ...
    module.exports.ensureView(db, type, opts, function(err) {
      ...
      module.exports.buildKeys(opts, function(err, value) {
        ...
        db.view(type, viewName, opts, function(err, body) {
          ...
          console.log('response from nano------>',body);
          cb(null, body.rows);
        });
      });
    });
  ...
  find: function(db, type, options, cb) {
    ...
    //support for querying by partial keys (_partial =true/false, defaults to false)
    if (options.where && options.where._partial) {
      opts._partial = options.where._partial;
      delete options.where._view;
    }
